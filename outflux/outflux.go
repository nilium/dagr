...
loop:
	...
	if flerr := w.flushWithCapacity(context.Background(), max, unlock); flerr != nil {
		if log := logger(); log != nil {
			log("Flush failed after reaching capacity=%d: %v", max, flerr)
		}
	}
}
...
func (w *Proxy) Transaction(fn WriteFunc) (err error) {
	wx := w.Writer()
	defer func() {
		if clerr := logclose(wx, "outflux transaction writer"); err == nil {
			err = clerr
		}
		w.flushExcess()
	}()
...
func (w *Proxy) WritePoint(key string, when time.Time, tags dagr.Tags, fields dagr.Fields) (n int64, err error) {
	if key == "" {
		if log := logger(); log != nil {
			log("Empty key in point")
		}
		return 0, dagr.ErrEmptyKey
	} else if len(fields) == 0 {
		if log := logger(); log != nil {
			log("No fields in point %q", key)
		}
...
func (w *Proxy) flushWithCapacity(ctx context.Context, capacity int, swapped func()) error {
	if ctx == nil {
		if log := logger(); log != nil {
			log("outflux: flushWithCapacity: context is nil")
		}
		ctx = context.TODO()
	}
...
retryLoop:
	for i := 0; i <= retries; i++ {
		...
		if !retry || err == context.Canceled {
			if log := logger(); log != nil {
				log("Failed sending payload of size=%d via %v - will not retry: %v", len(data), w.sender, err)
			}
			break retryLoop
		}
		...
		if log := logger(); log != nil {
			log("Error sending payload of size=%d via %v - will retry in %v: %v", len(data), w.sender, next, err)
		}
		select {
	...
	if err != nil {
		if log := logger(); log != nil {
			log("Failed to send payload of size=%d via %v: %v", len(data), w.sender, err)
		}
	}
